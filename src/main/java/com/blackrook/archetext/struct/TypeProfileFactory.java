...
import com.blackrook.archetext.annotation.ATIdentity;
...
public class TypeProfileFactory
{
	...
	public static class Profile<T>
	{
		...
		/** Method for getting type identity. */
		private MethodInfo identityGetterMethod;
		/** Method for setting type identity. */
		private MethodInfo identitySetterMethod;
		...
		private Profile(Class<? extends T> inputClass, MemberPolicy policy)
		{
			...
			this.identityGetterMethod = null;
			this.identitySetterMethod = null;
			this.publicFieldsByName = new HashMap<String, FieldInfo>(4);
			this.getterMethodsByName = new HashMap<String, MethodInfo>(4);
			this.setterMethodsByName = new HashMap<String, MethodInfo>(4);
			this.publicFieldsByAlias = new HashMap<String, FieldInfo>(4);
			this.getterMethodsByAlias = new HashMap<String, MethodInfo>(4);
			this.setterMethodsByAlias = new HashMap<String, MethodInfo>(4);
			...
			for (Field f : inputClass.getFields())
			{
				...
				if (f.getAnnotation(ATIdentity.class) != null)
					identityField = fi;
			}
			...
			for (Method m : inputClass.getMethods())
			{
				if (!policy.isIgnored(m) && isGetter(m) && !m.getName().equals("getClass"))
				{
					...
					if (m.getAnnotation(ATIdentity.class) != null)
						identityGetterMethod = mi;
				}
				else if (!policy.isIgnored(m) && isSetter(m, inputClass))
				{
					...
					if (m.getAnnotation(ATIdentity.class) != null)
						identitySetterMethod = mi;
				}
			}
		}
		
		public FieldInfo getIdentityField()
		{
			return identityField;
		}
		
		public MethodInfo getIdentityGetterMethod()
		{
			return identityGetterMethod;
		}
		
		public MethodInfo getIdentitySetterMethod()
		{
			return identitySetterMethod;
		}
